...
import 'package:value_listenables/src/debounce.dart';
import 'package:value_listenables/src/default.dart';
import 'package:value_listenables/src/disposable.dart';
import 'package:value_listenables/src/dispose.dart';
import 'package:value_listenables/src/future.dart';
import 'package:value_listenables/src/listen.dart';
import 'package:value_listenables/src/map.dart';
import 'package:value_listenables/src/single.dart';
import 'package:value_listenables/src/stream.dart';
import 'package:value_listenables/src/tap.dart';
import 'package:value_listenables/src/unique.dart';
import 'package:value_listenables/src/handle.dart';
import 'package:value_listenables/src/where.dart';
...
extension ValueListenableE<T> on ValueListenable<T> {
  ...
  /// Applicative lift
  IDisposableValueListenable<B> lift<A, B>(
          ValueListenable<B Function(A)> fn, ValueListenable<A> a) =>
  ...
  IDisposableValueListenable<T> andDispose(Object object) =>
      AndDisposeValueListenable(this, object);
  IDisposableValueListenable<T> andDisposeAll(Iterable<Object> objects) =>
      AndDisposeAllValueListenable(this, objects);
  ...
  IDisposableValueListenable<T1> fold<T1>(T1 initial, T1 Function(T1, T) fn) =>
      FoldValueListenable(this, initial, fn);
...
extension ValueListenableFunctionApply<U, T> on U Function(T) {
  ValueListenable<U> apply(ValueListenable<T> arg) =>
      arg.lift(asValueListenable, arg);
  ValueListenable<U> operator >>(ValueListenable<T> arg) => apply(arg);
}

extension ValueListenableFunctionReturn1<U, T> on U Function(T) {
  ValueListenable<U> Function(T) get ret =>
      (v) => SingleValueListenable(this(v));
}
...
extension ObjectToValueListenable<T> on T {
  @Deprecated(
      'use asValueListenable, because it conforms to the effective dart naming convention')
